...
Deshalb wird nur der Client in Dart implementiert (\ref{HTML5-Client in Dart}). Dabei taucht ein weiteres Problem auf: der socket.io-Websocket-Server entspricht nicht der HTML5-Websocket-API-Spezifikation und benötigt deshalb auf Clientseite zusätzliche Bibliotheken. Diese Bibliotheken stehen in Dart nicht zur Verfügung. Dart unterstützt Websocketverbindungen clientseitig mit dem Paket dart:html. Darin wird ein HTML5-Websocket erwartet.
...
\end{lstlisting}
Der Geo-Index ist zwingend erforderlich, damit nicht jede Anfrage des Servers an die Datenbank sämtliche Datensätze durchlaufen muss, um die Schiffe in einem bestimmten geographischen Ausschnitt zu finden. Den Aufbau und Unterhalt des Geo-Indexes übernimmt ebenfalls der ais\_client-Prozess.
\begin{lstlisting}[caption=Aufbau des Geo-Indexes in ais\_client.js]
  ...
  \end{lstlisting}
Dabei handelt es sich um einen zusammengesetzten Index, weil neben der Geo-Position auch die Geschwindigkeit und der Zeitpunkt des Empfangs der Nachricht Filterkriterien sind. 
In Listing \ref{lst:query Mongo} ist zu sehen, wie der Server-Child-Prozess worker.js Daten für die vom Client übermittelten Geo-Daten aus der MongoDb anfragt.
  \begin{lstlisting} [caption=query in worker.js, label=lst:query Mongo]
...
Der zweite Fall macht eine Verwaltung der Clients auf Serverseite notwendig. Die Serveranwendung muss bei jeder Positionsmeldung wissen, welche Clients benachrichtigt werden müssen. 
